...
export default function ResetPassword() {
  ...
  return (
    <AuthShell
      activeTab="login"
      hideTabs
      title={success ? <>Password <span className="accent">Reset.</span></> : <>Set New <span className="accent">Password.</span></>}
      subtitle={success ? 'Your password has been updated. Redirecting to sign in...' : 'Create a strong password to secure your Pickup Zone account.'}
    >
      {success ? (
        <div className="pz-success">
          <div className="pz-success-circle"><Check aria-hidden="true" /></div>
          <div className="pz-success-title">Password reset</div>
        ...
        <form className="pz-auth-form" onSubmit={handleSubmit}>
          ...
          <button className="pz-submit" type="submit" disabled={loading || !passwordsMatch || !password}>
            {loading ? <><span className="pz-spinner" /> Resetting...</> : <>Reset Password <ArrowRight aria-hidden="true" size={16} /></>}
          </button>
